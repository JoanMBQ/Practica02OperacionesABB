...
public class ArbolGrafico extends JPanel {
    ...
    /**
     * Constructor de la clase ArbolGrafico. inicializa los atributos de la 
     * clase ArbolGrafico y llama al método repaint().
     * @param miArbol: dato de tipo Arbol que contiene el Arbol a dibujar.
     */
    public ArbolGrafico(Arbol miArbol) {
        Color colorFondo = new Color(204,204,255);
     ...
     * @param n:Objeto de la clase Nodo
     ...
     * @param n: Objeto de tipo Nodo que se utiliza como referencia para
    ...
    private void calcularPosicion(Nodo n, int left, int right, int top) {
        ...
        posicionNodos.put(n, new Rectangle(center - width / 2 - 3, top, width + 
                6, fm.getHeight()));

        calcularPosicion(n.getIzq(), Integer.MAX_VALUE, center - child2child / 2,
                top + fm.getHeight() + parent2child);
        calcularPosicion(n.getDer(), center + child2child / 2, Integer.MAX_VALUE,
                top + fm.getHeight() + parent2child);
    }
     ...
     * Nodo.
     * @param n: Objeto de la clase Nodo que se utiliza como referencia
